 ...
 */
public class FarmMultiBoxTracker {
...
//    };
    ...
    private List<FarmClassifier.Recognition> keyPointsResult;
    ...
    public FarmMultiBoxTracker(final Context context) {
    ...
    public synchronized void trackResults(
            final List<FarmClassifier.Recognition> results, final byte[] frame, final long timestamp) {
    ...
    public synchronized void draw(final Canvas canvas, int animalType) {
        ...
        int canvasH = ScreenUtil.getScreenHight();
...
//        canvas.drawRect(canvas.getWidth() * 0.15f, frameWidth * 0.18f, canvas.getWidth() * 0.85f, frameWidth * 0.85f, boxPaint);
        ...
        //判断是什么动物画框
        switch(animalType){
            case ANIMAL_TYPE_PIG:
                //;
                //drawCowBorder(canvas);
                break;
            case ANIMAL_TYPE_CATTLE:
                //drawCowBorder(canvas);
                break;
            case ANIMAL_TYPE_DONKEY:
                //...;
                //drawCowBorder(canvas);
                break;
            default:
        ...
        for (TrackerItem item : mFrameRects) {
            ...
            String tempAngle = "未知";
            if (FarmDetectorActivity.AngleTrackType == 1) {
                tempAngle = "左脸";
            } else if (FarmDetectorActivity.AngleTrackType == 2) {
                tempAngle = "正脸";
            } else if (FarmDetectorActivity.AngleTrackType == 3) {
                tempAngle = "右脸";
            } else if (FarmDetectorActivity.AngleTrackType == 10) {
                tempAngle = "未识别角度";
            }else {
                tempAngle = "未识别角度";
            }
    ...
    private void processResults(
            final long timestamp, final List<FarmClassifier.Recognition> results, final byte[] originalFrame) {
        final List<Pair<Float, FarmClassifier.Recognition>> rectsToTrack = new LinkedList<Pair<Float, FarmClassifier.Recognition>>();

        screenRects.clear();
        final Matrix rgbFrameToScreen = new Matrix(getFrameToCanvasMatrix());

        for (final FarmClassifier.Recognition result : results) {
            ...
            rectsToTrack.add(new Pair<Float, FarmClassifier.Recognition>(result.getConfidence(), result));
        }
        ...
        if (objectTracker == null) {
            ...
            for (final Pair<Float, FarmClassifier.Recognition> potential : rectsToTrack) {
    ...
    private void handleDetection(
            final byte[] frameCopy, final long timestamp, final Pair<Float, FarmClassifier.Recognition> potential) {
