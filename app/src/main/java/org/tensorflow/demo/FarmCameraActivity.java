...
public abstract class FarmCameraActivity extends BaseActivity implements OnImageAvailableListener {
    ...
    protected int maxLeft = 5;
    protected int maxMiddle = 5;
    protected int maxRight = 5;
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        //Toast.makeText(this, "FarmAppConfig.during="+FarmAppConfig.during, Toast.LENGTH_SHORT).show();

        setContentView(R.layout.farm_activity_camera);
        if (hasPermission()) {
            setFragment();
        } else {
            requestPermission();
        ...
        FarmDetectorActivity.type1Count=0;
        FarmDetectorActivity.type2Count=0;
        FarmDetectorActivity.type3Count=0;
...
//
    ...
    private void requestPermission() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (shouldShowRequestPermissionRationale(PERMISSION_CAMERA) || shouldShowRequestPermissionRationale(PERMISSION_STORAGE)) {
                Toast.makeText(FarmCameraActivity.this, "Camera AND storage permission are required for this apply", Toast.LENGTH_LONG).show();
            }
    ...
    protected void setFragment() {
        final Fragment fragment = FarmCameraConnectionFragment.newInstance(
                new FarmCameraConnectionFragment.ConnectionCallback() {
                    @Override
                    public void onPreviewSizeChosen(final Size size, final int rotation) {
                        previewHeight = size.getHeight();
                        previewWidth = size.getWidth();
                        FarmCameraActivity.this.onPreviewSizeChosen(size, rotation);
                    }
                },
        ...
        getFragmentManager()
                .beginTransaction()
                .replace(R.id.farm_container, fragment)
                .commit();
    }
