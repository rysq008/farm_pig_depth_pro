...
public class FaceDetector implements Classifier {
    ...
    @Override
    public List<Recognition> recognizeImage1(Bitmap bitmap, File file) {
        return null;
    }

    @Override
    public PostureItem recognizeImageDonkey(Bitmap bitmap, File detectedfile) {
        ...
        int padMinSize = Math.min(height, width);
...
//        resize bitmap
        Bitmap resized = Bitmap.createScaledBitmap(padBitmap, (int) 300, (int) 300, true);
        int[] resizedBitmapValues = new int[resized.getWidth() * resized.getHeight()];
        resized.getPixels(resizedBitmapValues, 0, resized.getWidth(), 0, 0, resized.getWidth(), resized.getHeight());

        sLogger.i("ScreenHight: " + ScreenUtil.getScreenHeight());
        sLogger.i("getScreenWidth: " + ScreenUtil.getScreenWidth());
        sLogger.i("height: " + height);
        sLogger.i("width: " + width);
        sLogger.i("resized.getWidth(): " + resized.getWidth());
        sLogger.i("resized.getHeight(): " + resized.getHeight());
        ...
        float[] inputResizedValues = new float[resized.getWidth() * resized.getHeight() * 3];
        ...
        for (int i = 0; i < resizedBitmapValues.length; i++) {
            inputResizedValues[i * 3 + 2] = ((float) ((resizedBitmapValues[i] & 0xFF) * 1.0));
            inputResizedValues[i * 3 + 1] = ((float) (((resizedBitmapValues[i] >> 8) & 0xFF) * 1.0));
            inputResizedValues[i * 3 + 0] = ((float) (((resizedBitmapValues[i] >> 16) & 0xFF) * 1.0));
        }

        inferenceInterface.feed(inputName, inputResizedValues, resized.getWidth(), resized.getHeight(), 3);

        long start1 = System.currentTimeMillis();
        inferenceInterface.run(outputNames);
        sLogger.i("donkey pb run cost: " + (System.currentTimeMillis() - start));
        float[] outputDetectionBoxes = new float[1 * 700];
        float[] outputDetectionScores = new float[1 * 100];

        inferenceInterface.fetch(outputNames[0], outputDetectionBoxes);
        inferenceInterface.fetch(outputNames[1], outputDetectionScores);

        for (int j = 0; j < 2; ++j) {
            sLogger.i("outputDetectionScores: %f" + outputDetectionScores[j]);
        }
        int thresholdNum = 0;
        ...
        float finalRotX = 0, finalRotY = 0;
        for (int i = 0; i < 100; i++) {
            if (outputDetectionScores[i] > MIN_CONFIDENCE) {
                thresholdNum++;
            } else {
                break;
            }
...
//            跟踪框使用位置
            float fy1_new0 = (float) outputDetectionBoxes[i * 7 + 0];
            float fx1_new0 = (float) outputDetectionBoxes[i * 7 + 1];
            float fy2_new0 = (float) outputDetectionBoxes[i * 7 + 2];
            float fx2_new0 = (float) outputDetectionBoxes[i * 7 + 3];

            //图片左上角在屏幕的X坐标
            int cutImgPositionX = 0;
            //图片左上角在屏幕的Y坐标
//            int cutImgPositionY = ScreenUtil.getScreenHight() / 6; //(ScreenUtil.getScreenHight() - ScreenUtil.getScreenWidth()) / 2;
            int cutImgPositionY = (ScreenUtil.getScreenHeight() - height) / 2;
            //图片宽度(图像像素)
            int cutImgWidth = width;
            //图片高度(图像像素)
            int cutImgHeight = height;
            //图像显示比例 (屏幕显示像素/图像像素)
            float imgDispRate = ScreenUtil.getScreenWidth() * 1.0f / width;
            //模型生成的x坐标(0.0-1.0)
...
//传给跟踪框画框函数
            ...
            fy1_new = (float) outputDetectionBoxes[i * 7 + 0] * height;
            fx1_new = (float) outputDetectionBoxes[i * 7 + 1] * width;
            fy2_new = (float) outputDetectionBoxes[i * 7 + 2] * height;
            fx2_new = (float) outputDetectionBoxes[i * 7 + 3] * width;

            float middle = (float) ((fx1_new + fx2_new) * 0.5);

            if ((Math.abs(middle - width * 0.5) < Math.abs(minVal - width * 0.5)
                    && (fx2_new - fx1_new >= 320) && (fy2_new - fy1_new >= 320))) {
                fy1 = fy1_new;
                fx1 = fx1_new;
                fy2 = fy2_new;
                fx2 = fx2_new;
                finalRotX = outputDetectionBoxes[i * 7 + 4];
                finalRotY = outputDetectionBoxes[i * 7 + 5];
                minVal = middle;
                scoreVal = outputDetectionScores[i];
            }
        }
        sLogger.i("fy1: %f", fy1);
        sLogger.i("fx1: %f", fx1);
...
//            srcy2 = pady2 + offsetY;
            ...
            sLogger.i("srcx1: %f", srcx1);
            sLogger.i("srcy1: %f", srcy1);
            sLogger.i("srcx2: %f", srcx2);
            sLogger.i("srcy2: %f", srcy2);
            sLogger.i("padBitmap_new_width: %f", padBitmap_new_width);
            sLogger.i("padBitmap_new_height: %f", padBitmap_new_height);
            //原比例图
            ...
            int width_zoom = 360, height_zoom = 480;
...
//            float multiple_big = 1.4f;
            ...
            //缩放到320*320的图片
            int width_big_zoom = 360, height_big_zoom = 480;
            ...
            // TODO: 2018/9/3 By:LuoLu  offset_X,offset_Y影响左右黑边；
            offset_X = (int) ((padBitmap_border_big - padBitmap_big.getWidth()) / 2.);
            offset_Y = (int) ((padBitmap_border_big - padBitmap_big.getHeight()) / 2.);
            padBitmap_final_big.setPixels(finalBitmapValues_big, 0, padBitmap_big.getWidth(), 0, offset_Y, padBitmap_big.getWidth(), padBitmap_big.getHeight());
            ...
            // TODO: 2018/9/4 By:LuoLu
            posture = new PostureItem(finalRotX, finalRotY, outputDetectionBoxes[6], positionMap1.get("X"), positionMap1.get("Y"), positionMap2.get("X"), positionMap2.get("Y"), padBitmap_zoom, padBitmap_big_zoom);
            sLogger.i("left top: " + positionMap1);
            sLogger.i("right bottom: " + positionMap2);

            Log.d(TAG, "getAngleCategory==rot_x" + finalRotX + "==rot_y==" + finalRotY);

        }
//            posture = new PostureItem(finalRotX, finalRotY, outputDetectionBoxes[6], 0, 0, ScreenUtil.getScreenWidth(), ScreenUtil.getScreenHight(), null , null);

        return posture;
    }

    // TODO: 2018/8/23 By:LuoLu  pig
    @Override
...
//            posture = new PostureItem(outputPredict[0], outputPredict[1], outputPredict[2], srcx11, srcy11, srcx22, srcy22, padBitmap_zoom, padBitmap_big_zoom);
    ...
    // TODO: 2018/8/23 By:LuoLu  cow
    @Override
    public PostureItem recognizeImageCow(Bitmap bitmap, File detectedfile) {
        PostureItem posture = null;
        if (bitmap == null) {
            return null;
        }
        ...
        bitmap.getPixels(bitmapValues, 0, bitmap.getWidth(), 0, 0, bitmap.getWidth(), bitmap.getHeight());
//        resize bitmap
        Bitmap resized = Bitmap.createScaledBitmap(bitmap, (int) 300, (int) 300, true);
        int[] resizedBitmapValues = new int[resized.getWidth() * resized.getHeight()];
        resized.getPixels(resizedBitmapValues, 0, resized.getWidth(), 0, 0, resized.getWidth(), resized.getHeight());
        ...
        long start1 = System.currentTimeMillis();
        inferenceInterface.run(outputNames);
        sLogger.i("cow pb run cost: " + (System.currentTimeMillis() - start));
        float[] outputDetectionBoxes = new float[1 * 700];
        float[] outputDetectionScores = new float[1 * 100];
...
//跟踪框位置信息
            ...
            //图片左上角在屏幕的Y坐标
            int cutImgPositionY = ScreenUtil.getScreenHeight() / 6;
//            int cutImgPositionY = (ScreenUtil.getScreenHight() - height) / 2;
            //图片宽度(图像像素)
            int cutImgWidth = width;
            //图片高度(图像像素)
            int cutImgHeight = height;
...
//            float multiple_big = 1.4f;
            ...
            Canvas canvas_big_zoom = new Canvas(padBitmap_big_zoom);
            canvas_big_zoom.drawBitmap(padBitmap_final_big, frameToCropTransform_big_zoom, null);
            posture = new PostureItem(finalRotX, finalRotY, outputDetectionBoxes[6], positionMap1.get("X"), positionMap1.get("Y"),
                    positionMap2.get("X"), positionMap2.get("Y"), padBitmap_zoom, padBitmap_big_zoom);
...
//            posture = new PostureItem(finalRotX, finalRotY, outputDetectionBoxes[6], 0, 0, ScreenUtil.getScreenWidth(), ScreenUtil.getScreenHight(), null , null);
    ...
    public static File saveImage1(Bitmap bmp) {
        File appDir = new File(new File(Environment.getExternalStorageDirectory(), "innovation"), "test92");
        if (!appDir.exists()) {
            appDir.mkdir();
        }
        String fileName = Calendar.getInstance().getTime() + ".jpeg";
    ...
    public static File saveImage2(Bitmap bmp) {
        File appDir = new File(new File(Environment.getExternalStorageDirectory(), "innovation"), "test724--2");
        if (!appDir.exists()) {
            appDir.mkdir();
        }
        String fileName = Calendar.getInstance().getTime() + ".jpeg";
    ...
    private float decodeExistEncoding(float[] outputExistEncoding) {
        if (outputExistEncoding == null || outputExistEncoding.length != 2) {
            return 0;
        }
        return (float) (Math.exp(outputExistEncoding[1] / outputExistEncoding[0]) + outputExistEncoding[1]);
    }
