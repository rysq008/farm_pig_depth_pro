...
public class UpdateReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        try {
             ...
             * 获取到当前的本地版本
             */
            UpdateInformation.localVersion = FarmAppConfig.getApplication().getPackageManager().getPackageInfo(FarmAppConfig.getApplication().getPackageName(), 0).versionCode;
            Log.e("onReceive", "onReceive: " + UpdateInformation.localVersion);
            /**
             * 获取到当前的版本名字
             */
            UpdateInformation.versionName = FarmAppConfig.getApplication()
                    .getPackageManager()
                    .getPackageInfo(
                            FarmAppConfig.getApplication().getPackageName(), 0).versionName;
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (model != null) {
...
//            UpdateInformation.serverVersion = 1959;
            //服务器标志
...
//            UpdateInformation.lastForce = 1957;
            //升级地址
            ...
            //检查版本
            checkVersion(FarmAppConfig.getActivity());
        }
