 ...
 */
public class YakFaceDetectTFlite implements FarmClassifier {
    private static final Logger sLogger = new Logger(YakFaceDetectTFlite.class);
    ...
    public static RecognitionAndPostureItem yakRecognitionAndPostureItemTFlite;
    ...
    public static FarmClassifier create(
            ...
            throws IOException {
        final YakFaceDetectTFlite d = new YakFaceDetectTFlite();
    ...
    @Override
    public RecognitionAndPostureItem cowRecognitionAndPostureItemTFlite(Bitmap bitmap, Bitmap originalBitmap) {
        return null;
    }
    ...
    @Override
    public RecognitionAndPostureItem yakRecognitionAndPostureItemTFlite(Bitmap bitmap, Bitmap oriBitmap) {
        String oriInfoPath = "";
        ...
        if (bitmap == null) {
            allNumber--;
            yakRecognitionAndPostureItemTFlite = null;
            return null;
        }

        yakRecognitionAndPostureItemTFlite = new RecognitionAndPostureItem();
        int height = bitmap.getHeight();
        int width = bitmap.getWidth();
        int padSize = height;
        SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmssSSS", Locale.getDefault());
        srcYakBitmapName = sdf.format(new Date(System.currentTimeMillis())) + ".jpeg";
        saveBitMap(bitmap,"yakSRC", srcYakBitmapName);
        ...
        // 输出头部识别图片
        saveBitMap(bitmap,"yakDetected_input", srcYakBitmapName);
        ...
        if (outputDetectNum[0] > 1){
            ...
            saveBitMap(bitmap,"yakDetected_ng3", srcYakBitmapName);
            if (dTimes < 6) {
                dTimes++;
            ...
            return yakRecognitionAndPostureItemTFlite;
        }
        ...
        if (outputDetectNum[0] < 1){
            ...
            FileUtils.saveInfoToTxtFile(oriInfoPath, srcYakBitmapName +
                    "；totalNum：" + allNumber + "；DetectTime：" + dTime + "；AngleTime：" + aTime + "；KeypointTime：" + kTime + "；totalTime：" + (dTime + aTime + kTime));
            saveBitMap(bitmap,"yakDetected_ng4", srcYakBitmapName);
            if (dTimes < 6) {
                dTimes++;
        ...
        if (outputScores[0][0] > 1 || outputScores[0][0] < MIN_CONFIDENCE) {
            ...
            FileUtils.saveInfoToTxtFile(oriInfoPath, srcYakBitmapName +
                    "；totalNum：" + allNumber + "；DetectTime：" + dTime + "；AngleTime：" + aTime + "；KeypointTime：" + kTime + "；totalTime：" + (dTime + aTime + kTime));
            saveBitMap(bitmap,"yakDetected_ng2", srcYakBitmapName);
            if (dTimes < 6) {
                dTimes++;
        ...
        if (left < 0 || top < 0 || right > preWidth - 2 * offsetY || bottom > preWidth - 2 * offsetX) {
            ...
            saveBitMap(bitmap,"yakDetected_ng5", srcYakBitmapName);
            FileUtils.saveInfoToTxtFile(oriInfoPath, srcYakBitmapName +
                    "；totalNum：" + allNumber + "；DetectTime：" + dTime + "；AngleTime：" + aTime + "；KeypointTime：" + kTime + "；totalTime：" + (dTime + aTime + kTime));
            if (dTimes < 6) {
                dTimes++;
        ...
        yakRecognitionAndPostureItemTFlite.setList(recognitions);

        saveBitMap(bitmap,"yakDetected_ok", srcYakBitmapName);
        //clip image
        ...
        yakRecognitionAndPostureItemTFlite.setPostureItem(posture);
        AnimalClassifierResultIterm.yakAngleCalculateTFlite(yakRecognitionAndPostureItemTFlite.getPostureItem());
