 ...
 */
public class CowFaceDetectTFlite implements FarmClassifier {
    private static final Logger sLogger = new Logger(CowFaceDetectTFlite.class);
    private static final boolean DEBUG = false;
    public static float MIN_CONFIDENCE;
    private static final String TAG = "CowFaceDetectTFlite";
    ...
    private static final String COW_TFLITE_PREDICTION_MODEL_FILE = "cow_rotation_1030_tf10.tflite";
    private static final String COW_TFLITE_KEYPOINTS_MODEL_FILE = "cow_keypoint_1029_tf10.tflite";
    ...
    private FarmClassifier cowFaceRotationDetector;
    private FarmClassifier cowFaceKeyPointsDetector;
    ...
    private CowFaceDetectTFlite() {
        try {
            cowFaceRotationDetector =
                    CowRotationPrediction.create(
                            FarmAppConfig.getApplication().getAssets(),
                            COW_TFLITE_PREDICTION_MODEL_FILE,
                            "",
                            192,
                            true);
            cowFaceKeyPointsDetector =
                    CowKeyPointsDetectTFlite.create(
                            FarmAppConfig.getApplication().getAssets(),
                            COW_TFLITE_KEYPOINTS_MODEL_FILE,
                            "",
                            192,
                            true);
    ...
    public static FarmClassifier create(
            ...
            throws IOException {
        final CowFaceDetectTFlite d = new CowFaceDetectTFlite();
    ...
    public RecognitionAndPostureItem cowRecognitionAndPostureItemTFlite(Bitmap bitmap, Bitmap oriBitmap) {
        ...
        if (bitmap == null) {
            allNumber--;
            cowRecognitionAndPostureItemTFlite = null;
            return null;
        }
        ...
        saveBitMap(bitmap, "cowSRC", srcCowBitmapName);
        ...
        // 输出头部识别图片
        saveBitMap(bitmap, "cowDetected_input", srcCowBitmapName);
        ...
        if (outputDetectNum[0] > 1) {
            ...
            FileUtils.saveInfoToTxtFile(oriInfoPath, srcCowBitmapName +
                    "；totalNum：" + allNumber + "；DetectTime：" + dTime + "；AngleTime：" + aTime + "；KeypointTime：" + kTime + "；totalTime：" + (dTime + aTime + kTime));
            saveBitMap(bitmap, "cowDetected_ng3", srcCowBitmapName);
            if (dTimes < 6) {
                dTimes++;
        ...
        if (outputDetectNum[0] < 1) {
            ...
            FileUtils.saveInfoToTxtFile(oriInfoPath, srcCowBitmapName +
                    "；totalNum：" + allNumber + "；DetectTime：" + dTime + "；AngleTime：" + aTime + "；KeypointTime：" + kTime + "；totalTime：" + (dTime + aTime + kTime));
            saveBitMap(bitmap, "cowDetected_ng4", srcCowBitmapName);
            if (dTimes < 6) {
                dTimes++;
        ...
        if (outputScores[0][0] > 1 || outputScores[0][0] < MIN_CONFIDENCE) {
            ...
            FileUtils.saveInfoToTxtFile(oriInfoPath, srcCowBitmapName +
                    "；totalNum：" + allNumber + "；DetectTime：" + dTime + "；AngleTime：" + aTime + "；KeypointTime：" + kTime + "；totalTime：" + (dTime + aTime + kTime));
            saveBitMap(bitmap, "cowDetected_ng2", srcCowBitmapName);
            if (dTimes < 6) {
                dTimes++;
        ...
        if (left < 0 || top < 0 || right > preWidth - 2 * offsetY || bottom > preWidth - 2 * offsetX) {
            ...
            saveBitMap(bitmap, "cowDetected_ng5", srcCowBitmapName);
            FileUtils.saveInfoToTxtFile(oriInfoPath, srcCowBitmapName +
                    "；totalNum：" + allNumber + "；DetectTime：" + dTime + "；AngleTime：" + aTime + "；KeypointTime：" + kTime + "；totalTime：" + (dTime + aTime + kTime));
            if (dTimes < 6) {
                dTimes++;
        ...
        saveBitMap(bitmap, "cowDetected_ok", srcCowBitmapName);
        //clip image
        ...
        PredictRotationIterm predictRotationIterm = cowFaceRotationDetector.cowRotationPredictionItemTFlite(clipBitmap, oriBitmap);
        ...
        aNumber++;
        Log.e(TAG, "aTime===" + aTime + "--aNumber===" + aNumber);

        // 调用模型判断关键点
        // keypoint detect
        kTime = System.currentTimeMillis();
        List<PointFloat> keypointResults = cowFaceKeyPointsDetector.recognizePointImage(clipBitmap, oriBitmap);
        ...
        kNumber++;
        Log.e(TAG, "kTime===" + kTime + "--kNumber===" + kNumber);
        //  keypoint 的判断逻辑
        posture = new PostureItem(
                ...
                modelY1 * padSize, modelX1 * padSize, resizeClipBitmap, null, oriBitmap);
